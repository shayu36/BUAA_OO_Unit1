...
public class Parser {
    ...
    public Factor parseFactor() {
        if (lexer.peek().equals("(")) {
            ...
            || lexer.peek().equals("dy") || lexer.peek().equals("grad")) {
            return parseDerivativeFactor(lexer.peek());
            ...
            if (!lexer.isEnd() && lexer.peek().equals("{")) {
                ...
                lexer.next();
                return new RecursiveFunctionFactor(order, argument);
            } else {
                lexer.next();
                Factor argument = parseFactor();
                lexer.next();
    ...
    private Factor parseDerivativeFactor(String type) {
        lexer.next(); // dx / dy / grad
        lexer.next(); // (
        Expr expr = parseExpr();
        lexer.next(); // )
        return new DerivativeFactor(type, expr);
    }
